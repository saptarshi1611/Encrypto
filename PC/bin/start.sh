...
<window title="Authentic QR">
  ...
  <notebook labels="Generate|Verify">
	...
	<button>
	  ...
	  <action signal="clicked">clear; java securedqr.smit.utilities.GenQR "$FILE" "$FILE_DIRECTORY"</action>
	...
	<vbox>
	  ...
	  <vbox>
	    <button>
	      ...
	      <width>40</width>
	      <label>Verify</label>
	      <action signal="clicked">clear; java securedqr.smit.utilities.Verify "$XFILE" "$XSIG" "$PKEY"</action>
	    </button>
	  </vbox>
	 </vbox>
      </expander>
